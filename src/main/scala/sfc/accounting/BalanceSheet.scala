...
/** Aggregate banking-sector balance sheet — sum over all 7 per-bank BankStates.
  ...
  * (corporate bond absorption, insurance/NBFI asset allocation), and government
  * fiscal arithmetic. All mutation happens at the per-bank level in
  * `Banking.BankState`; this aggregate is derived, never written back.
  */
