...
object Banking:
  ...
  def lendingRate(bank: BankState, cfg: Config, refRate: Double): Double =
    if bank.failed then refRate + 0.50
      ...
      val carPenalty = if bank.car < sfc.config.Config.MinCar * 1.5 then
  ...
  def canLend(bank: BankState, amount: Double, rng: Random,
  ...
  def clearInterbank(banks: Vector[BankState], configs: Vector[Config],
                     rate: Double): Vector[BankState] =
  ...
  def checkFailures(banks: Vector[BankState], month: Int,
                    enabled: Boolean = sfc.config.Config.BankFailureEnabled,
                    ccyb: Double = 0.0): (Vector[BankState], Boolean) =
  ...
  def computeStandingFacilities(banks: Vector[BankState],
  ...
  def allocateQePurchases(banks: Vector[BankState],
                          qeTotal: Double): Vector[BankState] =
