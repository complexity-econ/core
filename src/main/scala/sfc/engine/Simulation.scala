...
object Simulation:
  ...
  def step(
  ...
  ): (World, Array[Firm.State], Option[Vector[Household.State]], Either[Vector[sfc.accounting.SfcCheck.IdentityError], Unit]) =
    ...
    val s2 = steps.LaborDemographicsStep.run(
      steps.LaborDemographicsStep.Input(
        marketWage = w.hh.marketWage,
        firms = firms,
        demographics = w.demographics,
        immigration = w.immigration,
        zusBalance = w.zus.fusBalance.toDouble,
        ppkBondHoldings = w.ppk.bondHoldings.toDouble,
    ...
    val s3 = steps.HouseholdIncomeStep.run(
      steps.HouseholdIncomeStep.Input(
        employed = employed,
        newWage = newWage,
        bdp = bdp,
        bdpActive = bdpActive,
    ...
    val s4 = steps.DemandStep.run(
      steps.DemandStep.Input(
        employed = employed,
        domesticCons = domesticCons,
        living = living,
        priceLevel = w.priceLevel,
        zusContributions = w.zus.contributions.toDouble,
        ...
        gvcEnabled = Config.GvcEnabled,
        gvcSectorExports = w.gvc.sectorExports,
    ...
    val s5 = steps.FirmProcessingStep.run(
      steps.FirmProcessingStep.Input(
        w = w,
        firms = firms,
        households = households,
        updatedHouseholds = updatedHouseholds,
        newWage = newWage,
        resWage = resWage,
        m = m,
    ...
    val s6 = steps.HouseholdFinancialStep.run(
      steps.HouseholdFinancialStep.Input(
        hhAgg = hhAgg,
    ...
    val s9 = steps.BankUpdateStep.run(
      steps.BankUpdateStep.Input(
        ...
        newZus = newZus,
        newPpk = newPpk,
        rawPpkBondPurchase = rawPpkBondPurchase,
        wageGrowth = wageGrowth,
        pitRevenue = pitRevenue,
        importAdj = importAdj,
        aggUnempBenefit = aggUnempBenefit,
        perBankHhFlowsOpt = perBankHhFlowsOpt,
        hhAgg = hhAgg,
        govPurchases = govPurchases,
        laggedInvestDemand = laggedInvestDemand,
        ioFirms = ioFirms,
        rewiredFirms = rewiredFirms,
        ...
        tourismImport = tourismImport,
        euCofin = euCofin,
        euProjectCapital = euProjectCapital,
        gdp = gdp,
        newMacropru = newMacropru,
        newInfl = newInfl,
        newPrice = newPrice,
        netDomesticDividends = netDomesticDividends,
        foreignDividendOutflow = foreignDividendOutflow,
        ...
        corpBondBankCoupon = corpBondBankCoupon,
        corpBondBankDefaultLoss = corpBondBankDefaultLoss,
    ...
    val s10 = steps.WorldAssemblyStep.run(
      steps.WorldAssemblyStep.Input(
        w = w,
        firms = firms,
        households = households,
        living = living,
        sectorCap = sectorCap,
        baseMinWage = baseMinWage,
        updatedMinWagePriceLevel = updatedMinWagePriceLevel,
        newWage = newWage,
        resWage = resWage,
        employed = employed,
        newImmig = newImmig,
        ...
        totalBondDefault = totalBondDefault,
        actualBondIssuance = actualBondIssuance,
        resolvedBank = s9.resolvedBank,
        finalBankingSector = s9.finalBankingSector,
        reassignedFirms = s9.reassignedFirms,
        reassignedHouseholds = s9.reassignedHouseholds,
        finalPpk = s9.finalPpk,
        finalInsurance = s9.finalInsurance,
        finalNbfi = s9.finalNbfi,
        newGovWithYield = s9.newGovWithYield,
        newJst = s9.newJst,
        housingAfterFlows = s9.housingAfterFlows,
        bfgLevy = s9.bfgLevy,
        bailInLoss = s9.bailInLoss,
        multiCapDestruction = s9.multiCapDestruction,
        monAgg = s9.monAgg,
        finalHhAgg = s9.finalHhAgg,
        vat = s9.vat,
        vatAfterEvasion = s9.vatAfterEvasion,
        pitAfterEvasion = s9.pitAfterEvasion,
        exciseRevenue = s9.exciseRevenue,
    ...
    (s10.newWorld, s10.finalFirms, s10.reassignedHouseholds, s10.sfcResult)
