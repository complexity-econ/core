...
object FirmProcessingStep:
  ...
  def run(in: Input): Output =
    ...
    val macro4firms = in.w.copy(
      month = in.m, sectorDemandMult = in.sectorMults,
    ...
    val corpBondAbsorption = CorporateBondMarket.computeAbsorption(
      in.w.corporateBonds, sumBondIssuance, in.w.bank.car, Config.MinCar)
    ...
    Output(
      ioFirms = ioFirms, finalHouseholds = finalHouseholds,
      sumTax = sumTax, sumCapex = sumCapex, sumTechImp = sumTechImp,
      sumNewLoans = sumNewLoans, sumEquityIssuance = sumEquityIssuance,
      sumGrossInvestment = sumGrossInvestment, sumBondIssuance = sumBondIssuance,
      sumProfitShifting = sumProfitShifting, sumFdiRepatriation = sumFdiRepatriation,
      sumInventoryChange = sumInventoryChange, sumCitEvasion = sumCitEvasion,
      sumEnergyCost = sumEnergyCost, sumGreenInvestment = sumGreenInvestment,
      totalIoPaid = totalIoPaid, nplNew = nplNew, nplLoss = nplLoss,
      totalBondDefault = totalBondDefault, firmDeaths = firmDeaths,
      intIncome = intIncome, corpBondAbsorption = corpBondAbsorption,
      actualBondIssuance = actualBondIssuance, netMigration = netMigration,
      ...
      lendingRates = lendingRates, postFirmCrossSectorHires = postFirmCrossSectorHires)
