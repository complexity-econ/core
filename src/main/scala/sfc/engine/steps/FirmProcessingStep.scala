...
object FirmProcessingStep:
  ...
  def run(in: Input): Output =
    ...
    val currentCcyb = in.w.macropru.ccyb
    ...
    // Materialize lending rates array for output
    ...
    // Corporate bond demand-side constraint
    ...
    // Net migration (for aggregate mode TotalPopulation update)
