          ...
          val hhBankRates = Some(BankRates(
            ...
            depositRates = in.w.bankingSector match
              case Some(bs) => bs.banks.map(_ =>
                BankingSector.hhDepositRate(in.w.nbp.referenceRate)).toArray
              case None => Array(BankingSector.hhDepositRate(in.w.nbp.referenceRate))
          ))
          val eqReturn = in.w.equity.monthlyReturn
