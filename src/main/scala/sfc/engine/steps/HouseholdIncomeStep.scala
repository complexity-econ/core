  ...
  def run(in: Input): Output =
    ...
    val (totalIncome, consumption, importCons, domesticCons, updatedHouseholds, hhAgg, perBankHhFlowsOpt) =
      in.households match
        case None =>
          val avgWageMult =
            if Config.UnionEnabled then
              SECTORS.indices
                .map(i => SECTORS(i).share.toDouble * (1.0 + Config.UnionWagePremium * Config.UnionDensity(i)))
                .sum
            else 1.0
          ...
          val ti =
            grossIncome - in.newZus.contributions.toDouble + in.newZus.pensionPayments.toDouble - pitDeduction + socialTransfers + aggUnempBenefit
