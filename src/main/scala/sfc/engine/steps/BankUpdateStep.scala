...
object BankUpdateStep:
  ...
  def run(in: Input): Output =
    ...
    val customsDutyRevenue = if Config.OeEnabled then
      in.newBop.totalImports.toDouble * Config.CustomsNonEuShare * Config.CustomsDutyRate
    else 0.0
    ...
    val socialTransferSpend = if Config.Social800Enabled then
      in.hhAgg.map(_.totalSocialTransfers.toDouble).getOrElse(
    ...
    val pitAfterEvasion = if Config.InformalEnabled then
      in.pitRevenue * (1.0 - effectiveShadowShare * Config.InformalPitEvasion) else in.pitRevenue

    val newGov = Sectors.updateGov(in.w.gov, in.sumTax + in.dividendTax + pitAfterEvasion, vatAfterEvasion,
      in.bdpActive, in.bdp, in.newPrice, unempBenefitSpend,
      in.monthlyDebtService, in.nbpRemittance, in.newZus.govSubvention.toDouble, socialTransferSpend,
      euCofinancing = in.euCofin, euProjectCapital = in.euProjectCapital,
    ...
    val mortgageBaseRate: Double = if Config.InterbankTermStructure then
      in.w.bankingSector.map(bs => YieldCurve.compute(bs.interbankRate.toDouble).wibor3m.toDouble)
    ...
    val housingAfterPrice = HousingMarket.step(in.w.housing, mortgageRate, in.newInfl,
      in.wageGrowth, in.employed, prevMortgageRate.toDouble)
    val housingAfterOrig = HousingMarket.processOrigination(housingAfterPrice, in.totalIncome,
      mortgageRate, true)
    ...
    val newBank = in.w.bank.copy(
      ...
      capital    = PLN(in.w.bank.capital.toDouble - in.nplLoss - mortgageDefaultLoss - in.consumerNplLoss
      ...
      deposits   = PLN(in.w.bank.deposits.toDouble + (in.totalIncome - in.consumption) + investNetDepositFlow
                   + jstDepositChange
      ...
      consumerLoans = PLN(Math.max(0.0, in.w.bank.consumerLoans.toDouble + in.consumerOrigination - in.consumerPrincipal - in.consumerDefaultAmt)),
      consumerNpl = PLN(Math.max(0.0, in.w.bank.consumerNpl.toDouble + in.consumerDefaultAmt - in.w.bank.consumerNpl.toDouble * 0.05)),
    ...
    val finalInsurance = in.newInsurance.copy(govBondHoldings = PLN(in.w.insurance.govBondHoldings.toDouble + insBondPurchase))
    ...
    val finalBank = if Config.GovBondMarket then
      newBank.copy(govBondHoldings = PLN(newBank.govBondHoldings.toDouble + actualBondChange - in.qePurchaseAmount - ppkBondPurchase - insBondPurchase - tfiBondPurchase))
    else newBank.copy(govBondHoldings = PLN(newBank.govBondHoldings.toDouble - in.qePurchaseAmount - ppkBondPurchase - insBondPurchase - tfiBondPurchase))
    ...
    val (finalBankingSector, reassignedFirms, reassignedHouseholds, bailInLoss, multiCapDestruction) = in.w.bankingSector match
      case Some(bs) =>
        ...
        val updatedBanks = bs.banks.map { b =>
          ...
          val (bankIncomeShare, bankConsShare, bankHhDebtService, bankDepInterest,
            ...
            in.perBankHhFlowsOpt match
              ...
              case None =>
                ...
                (in.totalIncome * ws, in.consumption * ws, in.hhDebtService * ws, 0.0,
                 in.consumerDebtService * ws, in.consumerOrigination * ws, in.consumerDefaultAmt * ws)
          ...
          val newLoansTotal = Math.max(0.0, b.loans.toDouble + in.perBankNewLoans(bId) - bankNplNew * Config.LoanRecovery)
          val ws = if totalWorkers > 0 then in.perBankWorkers(bId) / totalWorkers else 0.0
    ...
    Output(resolvedBank, finalBankingSector, reassignedFirms, reassignedHouseholds,
      finalPpk, finalInsurance, finalNbfi, newGovWithYield, newJst, housingAfterFlows,
      bfgLevy, bailInLoss, multiCapDestruction, monAgg, finalHhAgg,
      vat, vatAfterEvasion, pitAfterEvasion, exciseRevenue, exciseAfterEvasion,
      customsDutyRevenue, effectiveShadowShare,
      mortgageInterestIncome, mortgagePrincipal, mortgageDefaultLoss, mortgageDefaultAmount,
      jstDepositChange, investNetDepositFlow, actualBondChange)
