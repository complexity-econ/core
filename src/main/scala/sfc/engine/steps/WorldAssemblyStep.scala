...
object WorldAssemblyStep:
  ...
  def run(in: Input, rng: Random)(using p: SimParams): Output =
    ...
    val sfcFlows  = Sfc.buildMonthlyFlows(
      Sfc.BuildFlowsInput(
        unempBenefitSpend = in.s9.newGovWithYield.unempBenefitSpend,
        socialTransferSpend = in.s9.newGovWithYield.socialTransferSpend,
        govPurchases = PLN(in.s4.govPurchases),
        monthlyDebtService = in.s8.banking.monthlyDebtService,
        zusGovSubvention = in.s2.newZus.govSubvention,
        euCofin = PLN(in.s7.euCofin),
        sumCit = PLN(in.s5.sumTax),
        dividendTax = PLN(in.s7.dividendTax),
        pitAfterEvasion = PLN(in.s9.pitAfterEvasion),
        vatAfterEvasion = PLN(in.s9.vatAfterEvasion),
        nbpRemittance = in.s8.banking.nbpRemittance,
        exciseAfterEvasion = PLN(in.s9.exciseAfterEvasion),
        customsDutyRevenue = PLN(in.s9.customsDutyRevenue),
        nplLoss = PLN(in.s5.nplLoss),
        intIncome = PLN(in.s5.intIncome),
        hhDebtService = PLN(in.s6.hhDebtService),
        sumNewLoans = PLN(in.s5.sumNewLoans),
        nplNew = PLN(in.s5.nplNew),
        loanRecoveryRate = p.banking.loanRecovery,
        totalIncome = PLN(in.s3.totalIncome),
        consumption = PLN(in.s3.consumption),
        ...
        qePurchaseAmount = in.s8.monetary.qePurchaseAmount,
        govBondMarketEnabled = p.flags.govBondMarket,
        actualBondChange = PLN(in.s9.actualBondChange),
        netDomesticDividends = PLN(in.s7.netDomesticDividends),
        foreignDividendOutflow = PLN(in.s7.foreignDividendOutflow),
        mortgageInterestIncome = PLN(in.s9.mortgageInterestIncome),
        mortgageDefaultLoss = PLN(in.s9.mortgageDefaultLoss),
        mortgageOrigination = in.s9.housingAfterFlows.lastOrigination,
        mortgagePrincipal = PLN(in.s9.mortgagePrincipal),
        ...
        consumerPrincipal = PLN(in.s6.consumerPrincipal),
        consumerDefaultAmt = PLN(in.s6.consumerDefaultAmt),
        corpBondBankCoupon = in.s8.corpBonds.corpBondBankCoupon,
        corpBondBankDefaultLoss = in.s8.corpBonds.corpBondBankDefaultLoss,
        actualBondIssuance = PLN(in.s5.actualBondIssuance),
        corpBondAmort = in.s8.corpBonds.corpBondAmort,
        ...
        sumProfitShifting = PLN(in.s5.sumProfitShifting),
        sumFdiRepatriation = PLN(in.s5.sumFdiRepatriation),
        ...
        bailInLoss = PLN(in.s9.bailInLoss),
        multiCapDestruction = PLN(in.s9.multiCapDestruction),
