...
object SectoralMobility:
  ...
  /** Default 6x6 symmetric transition friction matrix.
    ...
    * BPO<->Retail low (digital/service overlap), Mfg<->Agr moderate (physical skills),
    * Healthcare high from non-medical (credential barrier). */
  ...
  def sectorVacancies(households: Vector[Household], firms: Array[Firm]): Array[Int] =
    ...
    for s <- 0 until 6 do
      vac(s) = Math.max(0, sectorDemand(s) - workerCounts(s))
    vac
  ...
  def sectorWages(households: Vector[Household]): Array[Double] =
    ...
    for s <- 0 until 6 do
      result(s) = if counts(s) > 0 then sums(s) / counts(s) else 0.0
    result

  /** Probabilistic target sector selection weighted by wage, vacancy, and inverse friction.
    * score(to) = avgWage(to) * (vacancies(to) + 1)^vacancyWeight * (1 - friction(from, to))
    * Returns selected sector index. */
  def selectTargetSector(from: Int, wages: Array[Double], vacancies: Array[Int],
                         matrix: Vector[Vector[Double]], vacancyWeight: Double,
                         rng: Random): Int =
    ...
    if total <= 0.0 then
      ...
      for to <- 0 until 6 do
        cum += scores(to)
        if cum >= r && scores(to) > 0 && selected == 0 && to != from then
          selected = to
      if selected == 0 then
        // Edge case: pick first non-from sector with score
        (0 until 6).find(to => to != from && scores(to) > 0).getOrElse(
          (0 until 6).find(_ != from).getOrElse(0)
        )
      else selected

  /** Adjust retraining parameters by friction level. */
  def frictionAdjustedParams(friction: Double, durationMult: Double,
                             costMult: Double): (Int, Double) =
    val adjDuration = Math.round(
      Config.HhRetrainingDuration * (1.0 + friction * durationMult)).toInt
