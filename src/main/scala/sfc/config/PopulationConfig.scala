...
/** Agent population and firm size distribution.
  ...
  *   workers per firm under `Uniform` distribution (also used as normalizing base for `Gus`)
  * @param firmSizeDist
  *   size distribution strategy: `Uniform` (equal sizes) or `Gus` (realistic micro/small/medium/large split)
  * @param firmSizeMicroShare
  *   share of micro-enterprises (1-9 employees) under `Gus` distribution (GUS CEIDG 2024: 96.2%)
