...
/** Mechanism toggles for 49 model features.
  *
  * Each flag enables/disables an independently switchable mechanism in the SFC-ABM engine. Defaults match current
  * `object Config` env-var defaults for PR 1 backward compatibility. In PR 2 all defaults flip to `true` (full Polish
  * economy baseline). Flags are used exclusively for counterfactual experiments — not for reproducing legacy behavior.
  *
  * Grouped by domain:
  *
  * '''Firm & production:''' `firmEntry`, `physCap`, `inventory`, `informal`, `energy`
  *
  * '''Government:''' `govInvest`, `euFunds`, `minWage`, `govBondMarket`, `govUnempBenefit`, `pit`, `social800`,
  * `social800ImmigEligible`
  *
  * '''Central bank (NBP):''' `nbpSymmetric`, `nbpStandingFacilities`, `nbpForwardGuidance`, `nbpQe`,
  * `nbpFxIntervention`
  *
  * '''Banking:''' `bankFailure`, `bankLcr`, `interbankTermStructure`, `creditDiagnostics`, `bailIn`, `macropru`, `jst`
  *
  * '''Social:''' `zus`, `ppk`, `demographics`
  *
  * '''Markets:''' `io`, `gpw`, `gpwEquityIssuance`, `gpwHhEquity`, `gpwDividends`
  *
  * '''External:''' `openEcon`, `gvc`, `immigration`, `immigEndogenous`, `fdi`, `remittance`, `tourism`
  *
  * '''Financial:''' `insurance`, `nbfi`, `re`, `reMortgage`, `reHhHousing`, `reRegional`
  *
