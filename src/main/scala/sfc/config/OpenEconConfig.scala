...
/** Open economy: trade elasticities, import content, net foreign assets, EU transfers, and FDI base.
  *
  * Extends the basic ForexConfig with detailed trade dynamics: per-sector import content, Marshall-Lerner elasticities,
  * exchange rate bands, ULC-based competitiveness, net foreign asset returns, EU structural transfers, and FDI base
  * flow. Calibrated to GUS/NBP 2024 balance of payments data.
  *
  * Stock values (`exportBase`, `euTransfers`, `fdiBase`) are in raw PLN — scaled by `gdpRatio` in `SimParams.defaults`.
  *
  * @param importContent
  *   per-sector import content of production (6 sectors, GUS supply-use tables 2024)
  * @param erFloor
...
case class OpenEconConfig(
  importContent: Vector[Double] = Vector(0.15, 0.50, 0.20, 0.15, 0.05, 0.12),
  erFloor: Double = 2.5,
  erCeiling: Double = 10.0,
  exportBase: Double = 138.5e9, // raw — scaled by gdpRatio
  importPushCap: Double = 0.03,
  ...
  nfaReturnRate: Double = 0.03,
  euTransfers: Double = 1.458e9, // raw — scaled by gdpRatio
  fdiBase: Double = 583.1e6, // raw — scaled by gdpRatio
