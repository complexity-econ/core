...
/** Input-Output matrix for inter-sectoral intermediate demand.
  ...
  * intermediate purchases from sector j. Inter-sector purchases are deposit transfers within the same bank (zero-sum
  * for total deposits), so they do not break existing SFC identities. Column sums are pre-computed for efficiency.
  *
  * Default matrix calibrated to GUS supply-use tables 2024.
  *
  * @param matrix
  *   6x6 technical coefficients matrix A[i][j] = sector i's input share from sector j
  * @param scale
  *   scaling factor for I-O flows (1.0 = full strength, for sensitivity analysis)
  */
case class IoConfig(
...
object IoConfig:
  /** Default 6x6 I-O technical coefficients matrix (GUS supply-use tables 2024).
    *
    * Rows/columns: BPO/SSC, Manufacturing, Retail/Services, Healthcare, Public, Agriculture.
    */
