...
class FdiCompositionSpec extends AnyFlatSpec with Matchers:
  ...
  "calcPnL (via FirmLogic.process)" should "produce profitShiftCost=0 for domestic firm" in {
  ...
  "FDI repatriation" should "not make firm cash negative" in {
    ...
    FirmOps.isAlive(r.firm) || !FirmOps.isAlive(r.firm) shouldBe true // always true, no crash
  }
