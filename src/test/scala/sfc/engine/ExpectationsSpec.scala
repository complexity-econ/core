...
class ExpectationsSpec extends AnyFlatSpec with Matchers:
  ...
  "Expectations.zero" should "have default values" in {
    ...
    z.expectedInflation shouldBe 0.025
    z.expectedRate shouldBe 0.0575
    z.credibility shouldBe 0.8
    z.forecastError shouldBe 0.0
  }

  "Expectations.initial" should "use config values" in {
    val i = Expectations.initial
    i.expectedInflation shouldBe Config.NbpTargetInfl
    i.expectedRate shouldBe Config.NbpInitialRate
    i.credibility shouldBe Config.ExpCredibilityInit
    i.forecastError shouldBe 0.0
    i.forwardGuidanceRate shouldBe Config.NbpInitialRate
  }
  ...
  "step" should "compute forecast error = realized - expected" in {
    ...
    r.forecastError shouldBe (0.05 - Config.NbpTargetInfl) +- 1e-10
  }
  ...
  it should "update expected rate adaptively without forward guidance" in {
    ...
    r.expectedRate should be > Config.NbpInitialRate
  }
  ...
  it should "use ECB target for EUR regime" in {
    ...
    r.credibility should be >= prev.credibility
  }

  // --- Stability ---

  it should "converge when inflation equals target persistently" in {
    var s = Expectations.initial.copy(credibility = 0.5)
    for _ <- 0 until 120 do
      s = Expectations.step(s, Config.NbpTargetInfl, Config.NbpInitialRate, 0.05, plnRc)
    s.credibility should be > 0.9
    s.expectedInflation shouldBe Config.NbpTargetInfl +- 0.005
    s.forecastError shouldBe 0.0 +- 0.005
  }
