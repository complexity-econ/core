...
class ReducedVatSpec extends AnyFlatSpec with Matchers:
  ...
  "VatRates" should "have 6 values" in {
    Config.VatRates.length shouldBe 6
  }

  it should "have all rates in [0, 0.23]" in {
    Config.VatRates.foreach { r =>
      r should be >= 0.0
      r should be <= 0.23
    }
  }
  ...
  "Weighted average" should "approximate real Polish effective VAT (~13%)" in {
    val weightedAvg = Config.FofConsWeights.zip(Config.VatRates).map((w, r) => w * r).sum
  ...
  "Sector ordering" should "have Healthcare and Agriculture rates below Retail" in {
    ...
    Config.VatRates(3) should be < Config.VatRates(2)
    Config.VatRates(5) should be < Config.VatRates(2)
  }

  it should "have BPO rate at standard 23%" in {
    Config.VatRates(0) shouldBe 0.23
  }

  "Sector-weighted VAT" should "be less than flat 23% for same consumption" in {
    val consumption = 1000000.0
    val sectorWeightedVat = consumption * Config.FofConsWeights.zip(Config.VatRates).map((w, r) => w * r).sum
  ...
  "Flat rates" should "reproduce old behavior when all rates are 0.23" in {
    ...
    val consumption = 1000000.0
    val flatVat = consumption * Config.FofConsWeights.zip(flatRates).map((w, r) => w * r).sum
  ...
  "Zero rates" should "produce zero VAT" in {
    ...
    val consumption = 1000000.0
    val vat = consumption * Config.FofConsWeights.zip(zeroRates).map((w, r) => w * r).sum
    vat shouldBe 0.0
  }

  "Default effective rate" should "be close to MF 2023 data (~13.9%)" in {
    val effectiveRate = Config.FofConsWeights.zip(Config.VatRates).map((w, r) => w * r).sum
