...
class LaborMarketSpec extends AnyFlatSpec with Matchers:
  ...
  "LaborMarket.separations" should "not change households when no firms changed" in {
    ...
    val hhs = (0 until 10).map(i =>
      mkHousehold(i, HhStatus.Employed(i % 5, 2, 8000.0))
    ).toVector
  ...
  it should "make workers unemployed when firm goes bankrupt" in {
    ...
    val hhs = Vector(
      ...
      mkHousehold(1, HhStatus.Employed(1, 2, 8000.0)),
      mkHousehold(2, HhStatus.Employed(2, 2, 8000.0))
    )
  ...
  it should "not affect already unemployed households" in {
    ...
    val hhs = Vector(
      ...
      mkHousehold(2, HhStatus.Employed(1, 2, 8000.0))
    )
    val result = LaborMarket.separations(hhs, prevFirms, newFirms)
  ...
  it should "prefer higher-skilled workers" in {
    ...
    val hhs = Vector(
      mkHousehold(0, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(1, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(2, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(3, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(4, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(5, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(6, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(7, HhStatus.Employed(0, 2, 8000.0)),
      mkHousehold(8, HhStatus.Employed(0, 2, 8000.0)),
  ...
  "LaborMarket.updateWages" should "produce mean wage = marketWage for employed" in {
    ...
    val hhs = Vector(
      mkHousehold(0, HhStatus.Employed(0, 2, 5000.0), skill = 0.8),
  ...
  it should "reduce relative wage with health penalty" in {
    ...
    val hhs = Vector(
      mkHousehold(0, HhStatus.Employed(0, 2, 5000.0), skill = 0.8, healthPenalty = 0.0),
      mkHousehold(1, HhStatus.Employed(1, 2, 5000.0), skill = 0.8, healthPenalty = 0.2)
    )
    val result = LaborMarket.updateWages(hhs, 10000.0)
  ...
  private def mkFirms(n: Int): Array[Firm] =
    (0 until n).map { i =>
      Firm(i, 50000.0, 0.0, TechState.Traditional(10), 0.5, 1.0, 0.5,
        2, Array.empty)  // sector 2 = Retail/Services
    }.toArray

  private def mkHousehold(id: Int, status: HhStatus,
                          skill: Double = 0.7, healthPenalty: Double = 0.0): Household =
    Household(id, 20000.0, 0.0, 1800.0, skill, healthPenalty, 0.82, status, Array.empty)
