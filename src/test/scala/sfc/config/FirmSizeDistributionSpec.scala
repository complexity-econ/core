...
class FirmSizeDistributionSpec extends AnyFlatSpec with Matchers:
  ...
  "FirmOps.skeletonCrew" should "return AutoSkeletonCrew for small firms" in {
    ...
    val f = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(5), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(0), Array.empty[Int], initialSize = 5)
    FirmOps.skeletonCrew(f) shouldBe Config.AutoSkeletonCrew
  }

  it should "scale with initialSize for large firms" in {
    import sfc.agents.{Firm, FirmOps, TechState}
    val f = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(250), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(0), Array.empty[Int], initialSize = 250)
  ...
  it should "be at least AutoSkeletonCrew" in {
    ...
    for size <- Vector(1, 3, 5, 10, 50, 100, 250, 500) do
      val f = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(size), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(0), Array.empty[Int], initialSize = size)
  ...
  "FirmOps.capacity" should "scale linearly with initialSize at full employment" in {
    import sfc.agents.{Firm, FirmOps, TechState}
    val f10 = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(10), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(2), Array.empty[Int], initialSize = 10)
    val f25 = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(25), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(2), Array.empty[Int], initialSize = 25)
  ...
  it should "give same per-worker revenue at full employment regardless of size" in {
    import sfc.agents.{Firm, FirmOps, TechState}
    val f5 = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(5), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(2), Array.empty[Int], initialSize = 5)
    val f100 = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(100), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(2), Array.empty[Int], initialSize = 100)
  ...
  "FirmOps.aiCapex" should "scale sublinearly with firm size" in {
    import sfc.agents.{Firm, FirmOps, TechState}
    val fSmall = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(10), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(2), Array.empty[Int], initialSize = 10)
    val fLarge = Firm(FirmId(0), PLN.Zero, PLN.Zero, TechState.Traditional(100), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(2), Array.empty[Int], initialSize = 100)
  ...
  "Default initialSize" should "be 10 for backward compatibility" in {
    ...
    val f = Firm(FirmId(0), PLN(50000), PLN.Zero, TechState.Traditional(10), Ratio(0.5), 1.0, Ratio(0.5), SectorIdx(0), Array.empty[Int])
