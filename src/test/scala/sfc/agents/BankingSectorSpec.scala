...
class BankingSectorSpec extends AnyFlatSpec with Matchers:
  ...
  "Banking.lendingRate" should "return high spread for failed bank" in {
    val bank = Banking.BankState(BankId(0), PLN(1e6), PLN(1e6), PLN(1e5), PLN(0), PLN(0), PLN(0), PLN(0), failed = true, 30, 0)
    val rate = Banking.lendingRate(bank, configs(0), 0.05)
    rate shouldBe (0.05 + 0.50) +- 0.001
  }

  it should "increase with NPL ratio" in {
    val bankLowNpl = Banking.BankState(BankId(0), PLN(1e6), PLN(1e6), PLN(2e5), PLN(1e4), PLN(0), PLN(0), PLN(0), false, 0, 0)
    val bankHighNpl = Banking.BankState(BankId(0), PLN(1e6), PLN(1e6), PLN(2e5), PLN(2e5), PLN(0), PLN(0), PLN(0), false, 0, 0)
  ...
  it should "reject when projected CAR too low" in {
    ...
    val bank = Banking.BankState(BankId(0), PLN(1e6), PLN(100000.0), PLN(8000.0), PLN(0), PLN(0), PLN(0), PLN(0), false, 0, 0)
  ...
  it should "approach lombard rate when NPL is high" in {
    val banks = Vector(
      Banking.BankState(BankId(0), PLN(1e6), PLN(1e6), PLN(2e5), PLN(1e5), PLN(0), PLN(0), PLN(0), false, 0, 0),  // 10% NPL
  ...
  "Banking.checkFailures" should "not trigger when disabled" in {
    val banks = Vector(
      Banking.BankState(BankId(0), PLN(1e6), PLN(1e6), PLN(1000.0), PLN(0), PLN(0), PLN(0), PLN(0), false, 0, 5)  // Very low CAR
    )
  ...
  it should "trigger after 3 consecutive months of low CAR" in {
    val bank = Banking.BankState(BankId(0), PLN(1e6), PLN(1e6), PLN(1000.0), PLN(0), PLN(0), PLN(0), PLN(0), false, 0, 2)
  ...
  it should "reset consecutive counter when CAR recovers" in {
    val bank = Banking.BankState(BankId(0), PLN(1e6), PLN(1e6), PLN(2e5), PLN(0), PLN(0), PLN(0), PLN(0), false, 0, 2)  // CAR = 0.20 > MinCar
    val (result, anyFailed) = Banking.checkFailures(Vector(bank), 30, enabled = true)
    anyFailed shouldBe false
  ...
  "Banking.resolveFailures" should "transfer deposits to healthiest bank" in {
    val banks = Vector(
      Banking.BankState(BankId(0), PLN(500000.0), PLN(100000.0), PLN(50000.0), PLN(0), PLN(10000.0), PLN(0), PLN(0), false, 0, 0),
      Banking.BankState(BankId(1), PLN(300000.0), PLN(80000.0), PLN(0.0), PLN(0), PLN(5000.0), PLN(0), PLN(0), failed = true, 30, 3)
    )
  ...
  it should "have per-bank deltas summing to exactly deficit (residual-based)" in {
    ...
    val banks = (0 until 7).map(i =>
      Banking.BankState(BankId(i), PLN(1e6 / 7.0 * (i + 1)), PLN(0), PLN(1e5), PLN(0), PLN(1000.0 * (i + 1)), PLN(0), PLN(0), false, 0, 0)
    ).toVector
