...
class EducationSpec extends AnyFlatSpec with Matchers:
  ...
  "Household.education" should "default to 2 (Secondary)" in {
    val hh = Household(0, PLN(1000.0), PLN.Zero, PLN(1800.0), 0.5, 0.0, 0.85,
  ...
  "LaborMarket.separations" should "retain tertiary workers before primary in automated firms" in {
    ...
    val hhPrimary = Household(0, PLN(5000.0), PLN.Zero, PLN(1800.0), 0.5, 0.0, 0.85,
      HhStatus.Employed(FirmId(0), SectorIdx(0), PLN(6000.0)), Array.empty[Int], education = 0)
    val hhTertiary = Household(1, PLN(5000.0), PLN.Zero, PLN(1800.0), 0.4, 0.0, 0.85,
      HhStatus.Employed(FirmId(0), SectorIdx(0), PLN(5000.0)), Array.empty[Int], education = 3)
    val hhVocational = Household(2, PLN(5000.0), PLN.Zero, PLN(1800.0), 0.6, 0.0, 0.85,
  ...
  it should "use skill as tiebreaker within same education level" in {
    ...
    val hhLowSkill = Household(0, PLN(5000.0), PLN.Zero, PLN(1800.0), 0.3, 0.0, 0.85,
      HhStatus.Employed(FirmId(0), SectorIdx(0), PLN(6000.0)), Array.empty[Int], education = 2)
    val hhHighSkill = Household(1, PLN(5000.0), PLN.Zero, PLN(1800.0), 0.9, 0.0, 0.85,
      HhStatus.Employed(FirmId(0), SectorIdx(0), PLN(7000.0)), Array.empty[Int], education = 2)
    val hhMidSkill = Household(2, PLN(5000.0), PLN.Zero, PLN(1800.0), 0.5, 0.0, 0.85,
  ...
  "HouseholdInit.initialize" should "assign education to all households" in {
    val rng = new Random(42)
    val firms = Array(
      Firm(FirmId(0), PLN(1000000.0), PLN.Zero, TechState.Traditional(5), 0.5, 1.0, 0.5, SectorIdx(0), Array.empty[Int]),
      Firm(FirmId(1), PLN(1000000.0), PLN.Zero, TechState.Traditional(5), 0.5, 1.0, 0.5, SectorIdx(2), Array.empty[Int])
    )
  ...
  it should "clamp skill within education-specific range" in {
    val rng = new Random(42)
    val firms = Array(
      Firm(FirmId(0), PLN(1000000.0), PLN.Zero, TechState.Traditional(10), 0.5, 1.0, 0.5, SectorIdx(0), Array.empty[Int])
    )
    val socialNet = Array.fill(10)(Array.empty[Int])
