...
class NetworkPropertySpec extends AnyFlatSpec with Matchers with ScalaCheckPropertyChecks:
  ...
  "localAutoRatio" should "be in [0, 1]" in {
    val firms = (0 until 10).map { i =>
      ...
      Firm(FirmId(i), PLN(100000), PLN.Zero, tech, 0.5, 1.0, 0.4, SectorIdx(0), (0 until 10).filter(_ != i).toArray)
    }.toArray
    for f <- firms do
  ...
  it should "be 0 when no neighbors" in {
    val firm = Firm(FirmId(0), PLN(100000), PLN.Zero, TechState.Traditional(10), 0.5, 1.0, 0.4, SectorIdx(0), Array.empty[Int])
